...
const endGame = () => {
  ...
  console.log('🔍 Cole - Verificando GameBridge:', {
  ...
  const trySaveProgress = (retries = 3) => {
    if (window.GameBridge && window.GameBridge.updateBestLevel) {
      console.log('💾 Cole - Guardando nivel en Firestore:', state.level);
      try {
        window.GameBridge.updateBestLevel('cole', state.level);
        console.log('✅ Cole - updateBestLevel llamado exitosamente');
      } catch (error) {
        console.error('❌ Cole - Error llamando updateBestLevel:', error);
      }
    } else if (retries > 0) {
      console.log(`⏳ Cole - GameBridge no disponible, reintentando en 500ms... (${retries} intentos restantes)`);
      setTimeout(() => trySaveProgress(retries - 1), 500);
    } else {
      console.error('❌ Cole - GameBridge no disponible después de 3 intentos');
      console.log('🔍 Cole - window.GameBridge:', window.GameBridge);
      console.log('🔍 Cole - updateBestLevel method:', window.GameBridge?.updateBestLevel);
    }
  };
