...
import { getCandies, addCandies, getBestParque, setBestParque } from './storage.js';
...
// Estado del juego - DISPONIBLE GLOBALMENTE
window.state = {
  distance: 0,
...
const gameLoop = () => {
  ...
  if (currentMilestone > lastMilestone) {
    ...
    updateHUD();
    toast(`📏 ${Math.floor(state.distance)}m = 🍬 ¡Golosina!`, 1500);
  ...
  // Nivel 1: 0-299m, Nivel 2: 300-599m, Nivel 3: 600-899m, etc.
  const newLevel = Math.max(1, Math.floor(state.distance / 300) + 1);
  
  // LIMPIEZA: Si el nivel es mayor que 10, algo está mal, resetear
  if (newLevel > 10) {
    console.log('⚠️ Nivel demasiado alto detectado:', newLevel, 'Reseteando a 1');
    state.level = 1;
    // Resetear también en Firestore
    if (window.GameBridge && window.GameBridge.resetProgress) {
      window.GameBridge.resetProgress('skate');
    }
    return; // Salir de la función para evitar más problemas
  }
  ...
  if (newLevel > state.level) {
    ...
    console.log(`🎯 Nivel ${state.level} alcanzado! Distancia: ${state.distance}m`);
  }
  
  // DEBUG: Mostrar nivel actual en pantalla
...
const endGame = () => {
  ...
  if (isNewRecord) {
    ...
    // Ya no usamos saveScoreToServer - usamos GameBridge.updateBestLevel
  }
  
  // Calcular nivel correcto basado en distancia (100 metros = 1 nivel)
  const calculatedLevel = Math.floor(finalDistance / 100) + 1;
  console.log('🎯 Skate - Distancia:', finalDistance, 'Nivel calculado:', calculatedLevel);
  
  // LIMPIEZA: Si el nivel es mayor que 10, algo está mal, resetear a 1
  if (calculatedLevel > 10) {
    console.log('⚠️ Nivel demasiado alto detectado:', calculatedLevel, 'Reseteando a 1');
    const resetLevel = 1;
    ...
    localStorage.setItem('aray_best_level_skate', resetLevel.toString());
  } else {
    // Guardar nivel correcto en localStorage
    localStorage.setItem('aray_best_level_skate', calculatedLevel.toString());
  }
  
  // Guardar nivel en Firestore con retry
  console.log('🔍 Skate - Verificando GameBridge:', {
  ...
  const trySaveProgress = (retries = 3) => {
    if (window.GameBridge && window.GameBridge.updateBestLevel) {
      const levelToSave = calculatedLevel > 10 ? 1 : calculatedLevel;
      console.log('💾 Skate - Guardando nivel en Firestore:', levelToSave);
      try {
        window.GameBridge.updateBestLevel('skate', levelToSave);
        console.log('✅ Skate - updateBestLevel llamado exitosamente');
      } catch (error) {
        console.error('❌ Skate - Error llamando updateBestLevel:', error);
      }
    } else if (retries > 0) {
      console.log(`⏳ Skate - GameBridge no disponible, reintentando en 500ms... (${retries} intentos restantes)`);
      setTimeout(() => trySaveProgress(retries - 1), 500);
    } else {
      console.error('❌ Skate - GameBridge no disponible después de 3 intentos');
      console.log('🔍 Skate - window.GameBridge:', window.GameBridge);
      console.log('🔍 Skate - updateBestLevel method:', window.GameBridge?.updateBestLevel);
    }
  };
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Leer mejor nivel desde Firestore (no localStorage)
  let bestLevel = parseInt(localStorage.getItem('aray_best_level_skate')) || 1;
  
  // Función para manejar el nivel recibido desde Firestore
  window.onBestLevelReceived = function(gameId, level) {
    if (gameId === 'skate') {
      console.log('📊 Mejor nivel recibido desde Firestore:', level);
      if (level > 0) {
        bestLevel = level;
  ...
  // Solicitar el mejor nivel desde Firestore
  if (window.GameBridge && window.GameBridge.getBestLevel) {
    console.log('📊 Solicitando mejor nivel desde Firestore...');
    window.GameBridge.getBestLevel('skate');
  } else {
    console.log('📊 GameBridge no disponible, usando localStorage:', bestLevel);
  }
...
// Función global para resetear todo el progreso
window.resetAllProgress = function() {
  console.log('🔄 Reseteando todo el progreso...');
  
  // Resetear localStorage
  localStorage.setItem('aray_best_level_skate', '1');
  ...
  console.log('✅ Progreso reseteado a nivel 1');
  alert('Progreso reseteado a nivel 1');
};
