...
const endGame = (reason = '👿 ¡Los demonios te alcanzaron!') => {
  ...
  console.log('🔍 Pabellón - Verificando GameBridge:', {
  ...
  const trySaveProgress = (retries = 3) => {
    if (window.GameBridge && window.GameBridge.updateBestLevel) {
      console.log('💾 Pabellón - Guardando nivel en Firestore:', state.wave + 1);
      try {
        window.GameBridge.updateBestLevel('pabellon', state.wave + 1);
        console.log('✅ Pabellón - updateBestLevel llamado exitosamente');
      } catch (error) {
        console.error('❌ Pabellón - Error llamando updateBestLevel:', error);
      }
    } else if (retries > 0) {
      console.log(`⏳ Pabellón - GameBridge no disponible, reintentando en 500ms... (${retries} intentos restantes)`);
      setTimeout(() => trySaveProgress(retries - 1), 500);
    } else {
      console.error('❌ Pabellón - GameBridge no disponible después de 3 intentos');
      console.log('🔍 Pabellón - window.GameBridge:', window.GameBridge);
      console.log('🔍 Pabellón - updateBestLevel method:', window.GameBridge?.updateBestLevel);
    }
  };
