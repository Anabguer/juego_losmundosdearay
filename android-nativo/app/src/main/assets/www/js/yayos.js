...
const endGame = (reason = '⏰ ¡Se acabó el tiempo!') => {
  ...
  console.log('🔍 Yayos - Verificando GameBridge:', {
  ...
  const trySaveProgress = (retries = 3) => {
    if (window.GameBridge && window.GameBridge.updateBestLevel) {
      console.log('💾 Yayos - Guardando nivel en Firestore:', state.level);
      try {
        window.GameBridge.updateBestLevel('yayos', state.level);
        console.log('✅ Yayos - updateBestLevel llamado exitosamente');
      } catch (error) {
        console.error('❌ Yayos - Error llamando updateBestLevel:', error);
      }
    } else if (retries > 0) {
      console.log(`⏳ Yayos - GameBridge no disponible, reintentando en 500ms... (${retries} intentos restantes)`);
      setTimeout(() => trySaveProgress(retries - 1), 500);
    } else {
      console.error('❌ Yayos - GameBridge no disponible después de 3 intentos');
      console.log('🔍 Yayos - window.GameBridge:', window.GameBridge);
      console.log('🔍 Yayos - updateBestLevel method:', window.GameBridge?.updateBestLevel);
    }
  };
