...
const config = {
  ...
  platformHeight: 15,
  platformGap: 180,
  gravity: 0.4, // Menos gravedad para salto más lento
  jumpPower: -12, // Salto más suave
  moveSpeed: 4 // Movimiento lateral más lento
};
...
const createPlatform = (width, absoluteY) => {
  ...
  if (absoluteY < -50) { // Plataformas móviles aparecen antes para poder verlas
    if (rand < 0.2) {
  ...
  const platform = {
    ...
    typeIndex: typeIndex,
    isMoving: isMoving,
    movingSpeed: isMoving ? 0.8 + Math.random() * 1.0 : 0, // Velocidad original de plataformas
...
const gameLoop = () => {
  ...
  state.platforms.forEach(platform => {
    if (platform.isMoving) {
      // Mover siempre; las Y ya están en coordenadas de pantalla tras el scroll
  ...
  if (state.player.vy > 0) {
    for (const platform of state.platforms) {
      if (state.player.x + state.player.size > platform.x &&
        ...
        // Sonido/vibración eliminados para evitar spam de consola y avisos del navegador
        // playSound('click');
        // vibrate(20);
      }
    }
  ...
  // Solo actualizar HUD cada 30 frames (0.5 segundos a 60fps) para mejorar rendimiento
  ...
  // Nivel cada 100 metros (más rápido de alcanzar)
...
const endGame = async (reason = '💥 ¡Caíste!') => {
  ...
  const audio = new Audio('audio/perder.mp3');
  audio.volume = 0.5;
...
const updateGameHUD = () => {
  ...
  // Cache de caramelos para evitar llamadas excesivas
  if (candiesEl) {
    const now = Date.now();
    if (!state.lastCandiesUpdate || (now - state.lastCandiesUpdate > 2000)) {
      state.cachedCandies = getCandies();
      state.lastCandiesUpdate = now;
    }
    candiesEl.textContent = state.cachedCandies || 0;
  }
};
