...
const updateAvatarPosition = () => {
  ...
  const cellW = mapEl.offsetWidth / MAP_WIDTH;
  const cellH = mapEl.offsetHeight / MAP_HEIGHT;
  
  // Posición del avatar relativa al mapa
  const x = (avatarPos.col * cellW) + (cellW / 2) - 25; // 25px = mitad del ancho del avatar
  const y = (avatarPos.row * cellH) + (cellH / 2) - 25; // 25px = mitad del alto del avatar

  console.log(`🎯 Avatar posición: fila=${avatarPos.row}, col=${avatarPos.col}, x=${x}, y=${y}`);
  console.log(`📏 Celda: width=${cellW}, height=${cellH}`);
...
const showRankingModal = async () => {
  ...
  if (!loggedIn) {
    content.innerHTML = `
      <div style="text-align:center; padding:20px;">
        ...
        <p style="font-size:1.1rem; color:#666; margin-bottom:2rem;">Inicia sesión o regístrate para guardar tus puntuaciones y aparecer en el ranking</p>
        ...
        <div id="auth-container">
          ...
          <div id="auth-buttons" style="display:flex; gap:1rem; justify-content:center; margin-bottom:2rem;">
            <button id="btn-show-login" class="btn-modal-play" style="background:#667eea;">Iniciar Sesión</button>
          ...
          <div id="login-form" style="display:none; background:#f8f9fa; padding:2rem; border-radius:12px;">
            <h3 style="margin-bottom:1rem; text-align:center;">Iniciar Sesión</h3>
            ...
            <input type="password" id="login-password" placeholder="Contraseña" style="width:100%; padding:12px; margin-bottom:1rem; border:2px solid #ddd; border-radius:8px; font-size:1rem;">
          ...
          <div id="register-form" style="display:none; background:#f8f9fa; padding:2rem; border-radius:12px;">
            ...
            <input type="password" id="register-password" placeholder="Contraseña (mín. 6 caracteres) *" style="width:100%; padding:12px; margin-bottom:1rem; border:2px solid #ddd; border-radius:8px; font-size:1rem;">
        ...
        <div style="margin-top:2rem; padding-top:2rem; border-top:2px solid #e0e0e0;">
          <p style="color:#888; font-size:0.9rem;">También puedes ver el ranking sin iniciar sesión</p>
    ...
    content.innerHTML = `
      <div style="text-align:center; padding:20px;">
        <h2 style="font-size:2rem; margin-bottom:0.5rem;">🏆 Ranking Global</h2>
        ...
        <button id="btn-logout" style="margin-top:1.5rem; padding:10px 20px; background:#ef4444; color:white; border:none; border-radius:8px; cursor:pointer;">Cerrar Sesión</button>
      </div>
    `;
  }
  ...
  if (!loggedIn) {
    ...
    // Usuario logueado - cargar ranking DESPUÉS de que el DOM esté listo
    console.log('Usuario logueado, preparando para cargar ranking...');
    requestAnimationFrame(() => {
      requestAnimationFrame(async () => {
        console.log('Cargando ranking ahora (después de 2 frames)...');
        await loadRankingList();
      });
    });
...
// Test GameBridge al cargar el mapa
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => {
    console.log('🧪 Probando GameBridge...');
    if (window.GameBridge) {
      console.log('✅ GameBridge está disponible');
      if (window.GameBridge.testGameBridge) {
        const result = window.GameBridge.testGameBridge();
        console.log('🧪 Test GameBridge resultado:', result);
      } else {
        console.log('❌ GameBridge.testGameBridge no disponible');
      }
      
      // Solicitar caramelos desde Firestore al cargar el mapa
      if (window.GameBridge.getCandiesAsync) {
        console.log('🍬 Solicitando caramelos desde Firestore...');
        window.GameBridge.getCandiesAsync();
      }
    } else {
      console.log('❌ GameBridge NO está disponible');
    }
  }, 2000);
});
